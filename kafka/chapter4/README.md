...
分区重分配的本质在于数据复制，先增加新的副本，然后进行数据同步，最后删除旧的副本来达到最终的目的。数据复制会占用额外的资源，如果重分配的量太大必然会影响
整体的性能。可通过减少重分配的粒度，以小批次的方式来操作是一种可行的思路。如果集群中某个主题或某个分区的流量在某段时间内特别大，那么只减少粒度是不足以应对的，这时需要
一个限流机制，对副本间的复制流量加以限制来确保重分配期间整体服务不会受太大影响

复制限流有两种实现方式：`kafka-config.sh`脚本和`kafka-reassign-partition.sh`脚本

### 4.3.4 修改副本因子

可以通过`kafka-reassign-partition.sh`脚本实现副本因子的增加和减少

## 4.4 如何选择合适的分区数

要根据实际的业务场景、软件条件、硬件条件、负载情况等来做具体的考量

### 4.4.1 性能测试工具

通过Kafka自带的`kafka-producer-perf-test.sh`和`kafka-consumer-perf-test.sh`脚本去进行测试
...
Kafka的分区数会受到Linux机器文件描述符数目限制。若是超过了允许的范围，Kafka会崩溃
