...
2. skiplist编码的有序集合对象
    ...
    - 有序集合中的每个元素的成员都是一个字符串对象，`zsl`与`dict`共享相同元素的成员和分值
    
    下图为`skiplist`编码的有序集合对象示意图：
    
    ![skiplist编码的有序集合对象](./8-17.png)
    ![有序集合元素同时被保存到字典和跳跃表中](./8-18.png)
    

### 6.1 编码转换

有序集合同时满足以下两个条件，对象使用`ziplist`编码：
- 有序集合保存的元素数量小于128个
- 有序集合保存的所有元素成员的长度都小于64字节

### 6.2 有序集合命令的实现

![有序集合命令的实现方式](./8-20.png)
![有序集合命令的实现方式](./8-21.png)
...
在执行一个类型特定的命令之前，Redis会通过`redisObject`结构的`type`属性来检查输入键类型是否正确，然后再决定是否执行给定的命令
...
![LLEN命令的执行过程](./8-22.png)

## 八、内存回收

C语言并不具备自动内存回收功能，所以Redis在自己的对象系统中构建了一个引用计数技术实现内存回收机制
...
Redis会在初始化服务器时，创建一万个字符串对象，这些对象包含了0到9999的所有整数值，当服务器需要用到0到9999的字符串对象时，服务器就会使用这些共享对象，而不是新创建对象。

创建共享字符串对象的数量可以通过修改`redis.h/REDIS_SHARED_INTEGERS`常量来修改

共享对象保存的值越复杂，验证共享对象和目标对象是否相同所需的复杂度就会越高，消耗的CPU时间也会越多

## 十、对象的空转时长

`redisObject`结构包含的`lru`属性记录了对象最后一次被命令程序访问的时间：
...
`OBJECT IDLETIME`命令可以打印给定键的空转时长，这一空转时长就是通过将当前时间减去键的值对象的lru时间计算得出的
...
- Redis共有字符串、列表、哈希、集合、有序集合五种类型的对象，每种类型的对象至少都有两种以上的编码方式，不同的编码可以在不同的场景上优化对象的使用效率
